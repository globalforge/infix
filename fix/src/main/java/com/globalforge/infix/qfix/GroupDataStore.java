...
public class GroupDataStore {
    private final static Logger logger = LoggerFactory
        .getLogger(GroupDataStore.class);
    /** A map of MsgType to repeating groups mapped by groupId */
    protected final Map<String, Map<String, RepeatingGroupBuilder>> messageGroups = new HashMap<String, Map<String, RepeatingGroupBuilder>>();
    /**
     * A map of repeating groupId to repeating group for groups defined outside
     * of message types
     */
    private final Map<String, RepeatingGroupBuilder> componentGroups = new HashMap<String, RepeatingGroupBuilder>();
    ...
    public boolean isMessageGroup(String curMessage, String groupId) {
        Map<String, RepeatingGroupBuilder> rgmap = messageGroups
            .get(curMessage);
        if (rgmap == null) {
            return false;
        }
    ...
    public void addMessageGroupMember(String curMessage, String groupId,
        String tagNum) {
        GroupDataStore.logger.info("Add Group Member: curMessage=" + curMessage
            + ", groupId=" + groupId + ", tagNum=" + tagNum);
        Map<String, RepeatingGroupBuilder> rgmap = messageGroups
            .get(curMessage);
